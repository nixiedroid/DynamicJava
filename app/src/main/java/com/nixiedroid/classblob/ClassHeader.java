...
public class ClassHeader extends ByteSerializable<ClassHeader> {
    ...
    public ClassHeader deserialize(ByteArrayInputStream stream) throws IOException {
        ...
        this.flags = new ClassHeaderFlags(this.converter.toInteger(stream.readNBytes(4),0, Endiannes.LITTLE));
        this.offset = this.converter.toInteger(stream.readNBytes(4),0,Endiannes.LITTLE);
        this.classSize = this.converter.toInteger(stream.readNBytes(4),0,Endiannes.LITTLE);
        if (this.flags.isFlagSet(ClassHeaderFlags.NAME_PROVIDED)) {
            if (stream.available() < 2) throw new IOException("Input is too short");
            this.nameLen = this.converter.toShort(stream.readNBytes(2),0,Endiannes.LITTLE);
        ...
        if (this.flags.isFlagSet(ClassHeaderFlags.ENCRYPTED) || this.flags.isFlagSet(ClassHeaderFlags.DECRYPTER)) {
            ...
            this.encAlgorithm = this.converter.toInteger(stream.readNBytes(4),0,Endiannes.LITTLE);
        }
        return this;
    }
    ...
    public void serialize(ByteArrayOutputStream stream) throws IOException {
        ...
        this.converter.fromInteger(arr,0,this.flags.toInt(),Endiannes.LITTLE);
        this.converter.fromInteger(arr,4,this.offset,Endiannes.LITTLE);
        this.converter.fromInteger(arr,8,this.classSize,Endiannes.LITTLE);
        stream.write(arr);
        if (this.flags.isFlagSet(ClassHeaderFlags.NAME_PROVIDED)) {
            arr = new byte[2];
            this.converter.fromShort(arr,0,this.nameLen,Endiannes.LITTLE);
            stream.write(arr);
        ...
        if (this.flags.isFlagSet(ClassHeaderFlags.ENCRYPTED) || this.flags.isFlagSet(ClassHeaderFlags.DECRYPTER)) {
            arr = new byte[2];
            this.converter.fromInteger(arr,0,this.encAlgorithm,Endiannes.LITTLE);
            stream.write(arr);
        }
    }
}
