...
class NativeChunkTest {
    ...
    public void createArray(){
        ...
        Assertions.assertEquals(size, canary.size());
        canary.free();
    }
    @Test
    public void testZero(){
        ...
        for (int i = 0; i < canary.size(); i++) {
            Assertions.assertEquals((byte)0, canary.get(i));
        }
        canary.free();
    }
    @Test
    public void testAddByte(){
        ...
        Assertions.assertEquals(canary.get(0),validValue);
        canary.free();
    }
    ...
    public void testToString(){
        NativeChunk canary = new NativeChunk(size);
        canary.set(0, validValue);
        Assertions.assertEquals(canary.toString(),"Native Chunk: [2a,00,00,00]");
        canary.free();
    }
    @Test
    ...
    public void testUseAfterFree(){
        NativeChunk canary = new NativeChunk(size);
        canary.set(0,validValue);
        Assertions.assertEquals(validValue,canary.get(0));
        canary.free();
